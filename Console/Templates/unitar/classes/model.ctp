...
$ds = ConnectionManager::getDataSource($useDbConfig);
$willUseTable = $ds->config['prefix'].$useTable;

if (SchemaSuite::existDefault($willUseTable)):
  foreach (SchemaSuite::getDefault($willUseTable) as $i => $default):
    if (isset($default['id'])||!isset($default['name'])) {
        break;
    }
    $o=$i+1;
    echo "\tconst ".Inflector::camelize(Inflector::slug($default['name']))." = $o;".PHP_EOL;
  endforeach;
endif;?>
